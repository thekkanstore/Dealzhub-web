...
import { getStoreById, getStoreBySlug, resolveOrAssignStoreSlug } from '../../services/storeFirestoreService';
...
import { Download, Store as StoreIcon, Phone, MapPin, Copy, Check } from 'lucide-react';
...
const StorePage = () => {
  ...
  useEffect(() => {
    ...
    if (store.slug) {
      ...
      const base = (store.storeName || 'shop')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '');
      setStoreSlug(base);

      resolveOrAssignStoreSlug(store).then((uniqueSlug) => {
        if (uniqueSlug) {
  ...
  const handleCopyLink = async () => {
    if (!storeSlug) return;
    const link = `https://dealzhub.co.in/shop/${storeSlug}`;
    try {
  ...
  // Generate QR Code
  useEffect(() => {
    if (!storeId || !isStoreOwner || !storeSlug) return;
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-md w-full text-center border border-gray-100 relative overflow-hidden">
          ...
          <div className="w-20 h-20 bg-gray-50 border border-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-xs">
            <StoreIcon className="w-10 h-10 text-gray-400" />
          </div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Store Unavailable</h1>
          <p className="text-gray-600 text-sm mb-6 leading-relaxed">
            This store is currently inactive or under maintenance and cannot be viewed at this time.
          </p>
          <button
            onClick={() => navigate('/home')}
            className="w-full bg-primaryButtonBackgroundColor text-white font-semibold py-3.5 px-6 rounded-full shadow-sm hover:shadow-md hover:bg-[#427256] transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] text-sm cursor-pointer"
          >
            Discover More Deals
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-md w-full text-center border border-gray-100 relative overflow-hidden">
          ...
          <div className="w-20 h-20 bg-rose-50 border border-rose-100 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-xs">
            <StoreIcon className="w-10 h-10 text-rose-500" />
          </div>
          <span className="inline-block px-3 py-1 bg-rose-100 text-rose-700 rounded-full text-xs font-bold mb-3">
            Store Registration Rejected
          </span>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Account Action Required</h1>
          <p className="text-gray-600 text-sm mb-6 leading-relaxed">
            This store application was reviewed and rejected by the administration. Please contact our support team or update your details.
          </p>
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={() => navigate('/home')}
              className="w-full py-3 px-6 bg-secondaryButtonBackgroundColor hover:bg-gray-200 text-gray-800 font-semibold rounded-full border border-gray-200 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] text-sm cursor-pointer"
            >
              Return to Home
            </button>
          </div>
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col">
        <div className='flex flex-col lg:flex-row justify-between items-start gap-8'>
          <div className='flex-1 min-w-0 w-full mb-2 lg:mb-0'>
            {/* Store Logo placed to the left of Store Name (Desktop & Mobile) */}
              ...
              <div className="shrink-0">
                {store.logoUrl || store.logo || store.storeLogo || store.imageUrl ? (
                  ...
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-2xl shadow-sm">
                    {store.storeName ? store.storeName.charAt(0).toUpperCase() : <StoreIcon className="w-8 h-8 text-[#528E6B]" />}
                  </div>
                )}
              </div>

              <div className="flex-1 min-w-0">
                <h1 className="text-2xl sm:text-3xl font-bold flex flex-wrap gap-2.5 items-center text-gray-900">
                  <span>{store.storeName}</span>
                  <span
                    className={`text-xs px-2.5 py-0.5 rounded-full border font-medium ${getStatusColor(store.vendorStatus)}`}
                  >
                    {store.vendorStatus}
                  </span>
                </h1>
                ...
                {store.bio && (
                  <p className="text-gray-700 text-sm mt-2.5 max-w-2xl leading-relaxed whitespace-pre-line bg-gray-50/80 p-3 rounded-xl border border-gray-100">
                    {store.bio}
                  </p>
                )}
              </div>
            ...
            {isStoreOwner && (
              <div className='flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 mt-4 w-full sm:w-auto'>
                <button
                  onClick={() => navigate('/editstore')}
                  className="w-full sm:w-auto h-11 px-6 bg-red-500/10 rounded-full flex items-center justify-center text-red-700 shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-300 ease-in-out hover:scale-[1.02] active:scale-[0.98] font-medium text-sm"
                >
                  Edit
                </button>
                {isPaymentPending ? (
                  <button
                    ...
                    className="w-full sm:w-auto h-11 px-6 bg-primaryButtonBackgroundColor text-white font-semibold rounded-full flex items-center justify-center border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 ease-in-out hover:scale-[1.02] active:scale-[0.98] text-sm"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full sm:flex sm:w-auto">
                    <button
                      ...
                      className="w-full sm:w-auto h-11 px-4 sm:px-6 bg-primaryButtonBackgroundColor text-white rounded-full flex items-center justify-center text-center font-medium text-xs sm:text-sm border border-gray-200 shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-300 ease-in-out hover:scale-[1.02] active:scale-[0.98] whitespace-nowrap"
                    >
                      Add Product
                    </button>
                    <button
                  ...
                  <span className="w-full sm:w-auto h-11 px-6 bg-amber-100 text-amber-800 border border-amber-200 rounded-full font-medium text-sm flex items-center justify-center shadow-sm">
            ...
            <div className="w-full lg:w-auto shrink-0 flex flex-col bg-gray-50 p-5 sm:p-6 rounded-xl border border-gray-200 shadow-sm">
              <div className="flex flex-wrap sm:flex-nowrap items-center justify-center sm:justify-start w-full gap-5 sm:gap-6">
                ...
                <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 shrink-0">
                  {qrCodeUrl ? (
                    <img src={qrCodeUrl} alt="Store QR Code" className="w-40 h-40 sm:w-44 sm:h-44" loading="lazy" />
                  ) : (
                    <div className="w-40 h-40 sm:w-44 sm:h-44 bg-gray-200 animate-pulse rounded-lg"></div>
                  )}
                </div>

                <div className="flex flex-col h-full py-1 min-w-0">
                  <div>
                    <h3 className="text-lg font-semibold sm:text-start text-center text-gray-800 truncate">{store.storeName} QR Code</h3>
                    <div className="mt-1.5 flex flex-wrap items-center sm:justify-start justify-center gap-2">
                      <span className="text-xs text-gray-600 font-mono bg-white px-2.5 py-1 rounded-md border border-gray-200 break-all select-all">
                        dealzhub.co.in/shop/{storeSlug}
                      </span>
                      <button
                        onClick={handleCopyLink}
                        type="button"
                        title={copied ? "Copied to clipboard" : "Copy shop link"}
                        aria-label="Copy shop link to clipboard"
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-xs font-medium transition-all duration-200 cursor-pointer shadow-2xs ${
                          copied
                            ? 'bg-emerald-50 border-emerald-300 text-emerald-700'
                            : 'bg-white border-gray-200 text-gray-600 hover:text-gray-900 hover:border-gray-300 hover:bg-gray-100'
                        }`}
                      >
                        {copied ? (
                          <>
                    ...
                    <p className="text-xs text-gray-600 sm:text-start text-center mt-2">Customers can scan to visit your shop</p>
                  </div>

                  <button
                    onClick={downloadQRCode}
                    className="mt-4 flex items-center justify-center gap-2 px-4 py-2.5 bg-primaryButtonBackgroundColor text-white rounded-full hover:shadow-md transition-all duration-300 ease-in-out hover:scale-[1.02] active:scale-[0.98] text-sm font-medium"
                  >
            ...
            {filteredProducts.length > 0 || isLoadingProducts ? (
              ...
              <div className="bg-white rounded-lg p-12 text-center">
                ...
                <div className="w-full flex items-center justify-center px-4">
                  {isStoreOwner ? (
                    currentStatus === 'pending' ? (
                      <span className="w-full max-w-xs sm:w-auto min-w-[200px] px-6 py-3 bg-amber-100 text-amber-800 rounded-full font-medium border border-amber-200 shadow-sm text-sm text-center">
                        Wait for approval to add product
                      </span>
                    ) : (
                      <button
                        onClick={() => navigate('/add-product')}
                        className="w-full max-w-xs sm:w-auto min-w-[200px] px-8 py-3 bg-primaryButtonBackgroundColor gap-2 rounded-full flex items-center justify-center text-white font-semibold border border-gray-200 shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-300 ease-in-out hover:scale-[1.02] active:scale-[0.98] text-sm whitespace-nowrap"
                      >
                        Add Product
                      </button>
                    )
